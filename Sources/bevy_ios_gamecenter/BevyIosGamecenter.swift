...
public func get_player(request: Int64) {
  Task {
    ...
    receive_player(
      request,
      IosGCPlayer.new(p.gamePlayerID, p.teamPlayerID, p.isAuthenticated, p.alias, p.displayName))
  }
}

private func convert_save_game(_ save: GKSavedGame) throws -> IosGCSaveGame {
  return IosGCSaveGame.new(
    save.name!, save.deviceName!, UInt64(try save.modificationDate!.timeIntervalSince1970))
}

private func convert_save_games(_ games: [GKSavedGame]) throws -> IosGCSaveGames {
...
public func save_game(request: Int64, data: RustString, name: RustString) {
  Task {
    do {
      ...
      let save: GKSavedGame = try await GKLocalPlayer.local.saveGameData(
        gameData, withName: name.toString())
...
public func resolve_conflicting_games(request: Int64, save_games: IosGCSaveGames, data: RustString)
{
  Task {
    do {
      ...
      receive_resolved_conflicts(
        request, IosGCResolvedConflictsResponse.done(try convert_save_games(result)))
    } catch {
      receive_resolved_conflicts(
        request, IosGCResolvedConflictsResponse.error(error.localizedDescription))
    }
  }
}
...
private func convert_achievement(_ a: GKAchievement) throws -> IosGCAchievement {
  return IosGCAchievement.new(
    a.identifier, a.percentComplete, a.isCompleted,
    UInt64(try a.lastReportedDate.timeIntervalSince1970))
}
...
public func achievement_progress(request: Int64, id: RustString, progress: Double) {
  Task {
    do {
      ...
      receive_achievement_progress(
        request, IosGCAchievementProgressResponse.error(error.localizedDescription))
    }
  }
}
...
public func reset_achievements(request: Int64) {
  Task {
    do {
      ...
      receive_achievement_reset(
        request, IosGCAchievementsResetResponse.error(error.localizedDescription))
    }
  }
}
  ...
  Task {
    do {
      try await GKLeaderboard.submitScore(
        Int(score), context: Int(context), player: GKLocalPlayer.local,
        leaderboardIDs: [id.toString()])
  ...
  Task {
    do {
      ...
      for leaderboard in leaderboard {
        let entry = try await leaderboard.loadEntries(
          for: [GKLocalPlayer.local], timeScope: GKLeaderboard.TimeScope.allTime)
        let score = Int32(entry.0?.score ?? 0)
        let rank = Int32(entry.0?.rank ?? 0)
        receive_fetch_leaderboard_score(
          request, IosGCLeaderboardFetchScoresResponse.done(score, rank))
      }
    } catch {
      receive_fetch_leaderboard_score(
        request, IosGCLeaderboardFetchScoresResponse.error(error.localizedDescription))
    }
  }
}

public func fetch_leaderboard_score_range(
  request: Int64, leaderboard_name: RustString, min: Int32, max: Int32
) {
  Task {
    do {
      let leaderboards = try await GKLeaderboard.loadLeaderboards(IDs: [leaderboard_name.toString()]
      )

      for leaderboard in leaderboards {
        let rangeStart = Swift.max(1, Int(min))
        let rangeLength = Swift.max(1, Int(max) - rangeStart + 1)
        ...
        var rustScores = RustVec<IosGCLeaderboardScore>()

        for entry in entries.1 {
          let rustScore = IosGCLeaderboardScore.new(
            entry.player.gamePlayerID,
            entry.player.displayName,
            Int32(entry.score),
            Int32(entry.rank)
          )
          rustScores.push(value: rustScore)
        }

        receive_fetch_leaderboard_score_range(
          request, IosGCLeaderboardFetchScoreRangeResponse.done(rustScores))
        return
      }

      // If no leaderboard found
      receive_fetch_leaderboard_score_range(
        request, IosGCLeaderboardFetchScoreRangeResponse.error("Leaderboard not found"))
    } catch {
      receive_fetch_leaderboard_score_range(
        request, IosGCLeaderboardFetchScoreRangeResponse.error(error.localizedDescription))
    }
  }
}
...
public func trigger_view(state: Int32) {
  GKAccessPoint.shared.trigger(
    state: GKGameCenterViewControllerState(rawValue: Int(state))
      ?? GKGameCenterViewControllerState.default
  ) {
...
public func fetch_signature(request: Int64) {
  Task {
    do {
      ...
      receive_items_for_signature_verification(
        request, IosGCFetchItemsForSignatureVerificationResponse.done(rust_items))
    } catch {
      receive_items_for_signature_verification(
        request, IosGCFetchItemsForSignatureVerificationResponse.error(error.localizedDescription))
    }
  }
}
